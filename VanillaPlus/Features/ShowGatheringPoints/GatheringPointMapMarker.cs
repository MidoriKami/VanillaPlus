...
public unsafe class GatheringPointMapMarker : MapMarkerNode {
    ...
    private uint GetIconId(uint gatheringPointId) {
        var gatheringPoint = IDataManager.Get().GetExcelSheet<GatheringPoint>().GetRow(gatheringPointId);
        var gatheringPointBase = IDataManager.Get().GetExcelSheet<GatheringPointBase>().GetRow(gatheringPoint.GatheringPointBase.RowId);
