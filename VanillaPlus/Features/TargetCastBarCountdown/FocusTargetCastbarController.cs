...
public class FocusTargetCastbarController : IAsyncDisposable {
    public TextNodeStyle LoadedStyle { get; }

    private readonly TargetCastBarCountdownConfig config;
    private TextNode? textNode;

    private const string StylePath = "TargetCastBarCountdown.FocusTarget.style.json";

    private readonly AddonController addonController;

    public unsafe FocusTargetCastbarController(TargetCastBarCountdownConfig config) {
        this.config = config;
    ...
    private unsafe void OnAddonSetup(AtkUnitBase* addon) {
        var targetNode = addon->GetNodeById(8);
    ...
    private unsafe void OnAddonRefresh(AtkUnitBase* addon) {
        if (IClientState.Get().IsPvP || !config.FocusTarget) {
