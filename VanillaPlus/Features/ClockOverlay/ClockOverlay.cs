...
public unsafe class ClockOverlay : GameModification {
    ...
    var defaultStyle = new TextNodeStyle {
        ...
        Position = config.Clock.Position,
    };

    clockStyle = Config.LoadConfig(stylePath, defaultStyle);

    if (clockStyle.Position == Vector2.Zero && config.Clock.Position != Vector2.Zero) {
        clockStyle.Position = config.Clock.Position;
    }
    ...
    Services.Framework.RunOnFrameworkThread(() => {
        if (config == null || overlayController == null) return;

        clockNode = new ClockNode(config.Clock, clockStyle) {
            Size = new Vector2(150.0f, 30.0f),
            Position = clockStyle.Position,
        };

        clockNode.OnMoveComplete = () => {
            config.Clock.Position = clockNode.Position;
