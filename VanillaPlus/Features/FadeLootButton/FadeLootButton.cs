...
public class FadeLootButton : GameModification {
    ...
    public override async Task OnEnableAsync() {
        ...
        configWindow.AddCategory(Strings.FadeLootButton_CategoryStyleSettings)
            .AddFloatSlider(Strings.FadeLootButton_LabelFadePercentage, 0.0f, 1.0f, 0.05f, nameof(config.FadePercent));
        ...
        await IFramework.Get().RunSafely(notificationLootController.Enable);
    }

    public override async Task OnDisableAsync() {
        await IFramework.Get().RunSafely(() => notificationLootController?.Dispose());
